...
def create_ensembles():
    ...
    for i in range(cnt):
        ...
        dates, closes, profits, close_rates, profits_rates, std, profit_product, ensemble_models, path = ep.main(conf, ep)
...
if __name__ == "__main__":
    ...
    conf.end_time = '2023/10/06/15:00'

    conf.reinfo_th = 0.089574609
    conf.pred_term = 27
    conf.reinfo_width = 15
    ...
    ep.last_trains, ep.start_times, ep.end_times, ep.term = ep.set_term(conf, ep.last_trains, ep.start_times, ep.end_times,
                            "_" + str(conf.reinfo_th) + "_" + str(conf.pred_term) + "_" + str(conf.reinfo_width))
