...
# 04월 03일
data.set_start_end_time(conf, '2024/04/01/09:00', '2024/04/03/15:00', '2024-03-31')
data.set_path(conf)
data.set_target_type(conf, 'C')
data.set_pred_term(conf, 5)
data.set_reinfo(conf, 0.0435646498507703) # reinfo_th,
data.set_ensemble(conf, ['20P', '30HL', '40C']) # selected_model_types
data.set_profit(conf, 0.005, 1) # loss_cut, profit_cut
data.set_width(conf, 65) # eval_width
...
if __name__ == "__main__":
    ...
    #prediction only
    conf.gubun = 0
    conf.result_path = 'pred_83_results2.csv'
