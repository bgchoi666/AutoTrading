...
# actor critic model load
actor_critic = tf.keras.models.load_model("H:/actor_critic_reinfo/알고리즘2-1/2024-01-01~2025-01-31")

def a2c_reinfo(conf, actor_critic):
    ...
    action_probs_t, values = actor_critic(pred_input)
...
def one_more_reinfo(conf):
    # 예측값을 한 번더 reinforcing

    # 에전 예측 결과 파일로부터 예측값 한 번 더 reinforcing
    pred_results = pd.read_csv(conf.result_path, encoding='euc-kr').values[:, :6]
    pred = pred_results[:, 1]
...
def main(conf, ep):
    ...
    for i in range(len(ep.last_trains)):
        ...
        if ep.every_term_random:
            ...
            conf.pred_term = random.randrange(5, 41, 5)#random.sample(eval_terms, 1)[0]
            conf.reinfo_width = random.randrange(10, 77)#random.sample(eval_widths, 1)[0]
    ...
    for k in range(len(ep.last_trains)-1):
        ...
        for n in range(5):
            if n == 0:
                ...
                i = random.randint(0, len(ep.last_trains)-1)
            conf.start_time = ep.start_times[i]
            conf.end_time = ep.end_times[i]
            ...
            if n == 0:
                ...
                avg_rates.append(np.array(profit_sum[-5:]).mean())
...
def predict(conf):
    ...
    if conf.reinfo_th > 0:
        ...
        mr.pred_term = conf.pred_term
        mr.target_type = conf.target_type
        mr.loss_cut = conf.loss_cut
        if conf.reinfo_mode == 'flex':
            pred = mr.reinfo_flex(pred, pred_results, conf.start_time, conf.reinfo_width)
        else:
            pred = mr.reinfo(pred, pred_results, conf.start_time, conf.reinfo_width)
        pred_results[:, 1] = np.array(pred)
