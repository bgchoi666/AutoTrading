...
if mode == 'mr2':
    #df = df.loc[df['알고리즘'] == '알2-1_random1년_2년_mr2'].reset_index(drop=True)
    #df = df.loc[df['알고리즘'] == '알2-1_1년_2년_mr2'].reset_index(drop=True)
    df = df.loc[df['알고리즘'] == '알2-1_random3개월_6개월_12개월_mr2'].reset_index(drop=True)

    #df = pd.concat([df1, df2], axis=0).reset_index(drop=True)
else:
    df = df.loc[df['알고리즘'] == '알2-1_random3개월_6개월_12개월_mr3'].reset_index(drop=True)
    #df = df.loc[df['알고리즘'] == '알2-1_random3개월_6개월'].reset_index(drop=True)
    #df = df.loc[df['알고리즘'] == '알2-1_1년_2년_mr3'].reset_index(drop=True)

for i in range(len(df)):
    df.loc[i, '최근탐색기간'] = df.loc[i, '최근탐색기간'].split('~')[1]
df = df.sort_values(by=['최근탐색기간'], axis=0).reset_index(drop=True)

df = df.loc[df['최근탐색기간'] >= start]
df = df.loc[df['최근탐색기간'] <= end].sort_values(by=['최근탐색기간'], axis=0).reset_index(drop=True)

term = df['최근탐색기간'].values

idx = 0
total = 0
for i in range(len(df)):

    if i != 0 and i < len(df) - 1 and term[i+1] <= last_trains[idx+1]:
        continue

    idx = np.where(last_trains >= term[i])[0][0]
    if last_trains[idx] >= end:
        break
    st = (datetime.datetime.strptime(last_trains[idx], "%Y-%m-%d") + datetime.timedelta(days=1)).strftime("%Y/%m/%d/09:00")
    if i < len(df) - 1 and term[i+1] > last_trains[idx+1] and len(last_trains) > idx+2:
        en = datetime.datetime.strptime(last_trains[idx + 2], "%Y-%m-%d").strftime("%Y/%m/%d/15:00")
    elif i == len(df) - 1:
        en = datetime.datetime.strptime(end, "%Y-%m-%d").strftime("%Y/%m/%d/15:00")
    else:
        en = datetime.datetime.strptime(last_trains[idx+1], "%Y-%m-%d").strftime("%Y/%m/%d/15:00")

    last_train = last_trains[idx]
    ...
    # actor critic에 의한 에측값 조정후 수익률 출력
...
print(start + "~" + end + " total: " + str(total))
