...
# model.py와의 차이는 매수, 매도, 중립 모든 target에 대하여 validation하면서 학습
...
def train(model, conf):
    ...
    while repeat_cnt < conf.max_repeat_cnt:
        ...
        # calculate accuracy
        c = 0
        s = list(prediction).count(1) + list(prediction).count(2) + list(prediction).count(0)
        for i in range(len(prediction)):
            if valid_y[i] == 2. and prediction[i] == 2.:
                    c += 1
...
def test(model, conf):
    ...
    # calculate accuracy
    c = 0
    s = list(test_prediction).count(1) + list(test_prediction).count(2) + list(test_prediction).count(0)
    for i in range(len(test_prediction)):
        if test_y[i] == 2 and test_prediction[i] == 2:
            c += 1
