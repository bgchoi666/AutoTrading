...
def calc_profit():
    ...
    for i in range(len(df)):
        ...
        if state == 1:
            ...
            if high - buy_price >= buy_price * loss_cut:
                ...
                count = 1
                buy_price = close
            elif pred == 0 and count == pred_term - 1:
                profit.append((buy_price - close)*250000)
                fee.append((buy_price + close)*250000*0.00003)
                state = pred
                count = 0
            ...
            if buy_price - low >= buy_price * loss_cut:
                ...
                #count = 1
                #buy_price = close
            elif pred == 1 or (count == pred_term-1 and pred == 0):
                profit.append((close - buy_price)*250000)
                fee.append((buy_price + close)*250000*0.00003)
                state = pred
                count = 1
    ...
    return (sum(profit) - sum(fee))/df['close'].values.mean()/250000/0.09+1
