...
et.selected_model_types = ['15P', '10C', '25HL']


#et.selected_model_types = random.sample(model_pools, et.selected_num)
#et.reinfo_th = random.sample([0.3, 0.4, 0.5], 1)[0]
#et.model_reinfo_th = et.reinfo_th
#et.pred_term = random.sample([10, 20 , 40], 1)[0]
...
term = datetime.strptime(start_time, "%Y/%m/%d/%H:%M").strftime("%Y-%m-%d") + "~" + \
       ...
       str(et.pred_term) + et.target_type + "_" + str(et.reinfo_th)
...
if __name__ == '__main__':
    ...
    #results.append([et.selected_model_types, et.reinfo_th, et.pred_term, profit_rates[-1], profits[1], profits[2]])

    #pd.DataFrame(np.array(results),
    #             columns=['ensemble', 'self-reflection', 'pred_term', 'profit_rates', 'std', 'profit_product']).to_csv(result_path, index=False)
