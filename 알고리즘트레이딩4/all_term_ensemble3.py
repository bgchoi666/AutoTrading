...
["15HL", "20P", "25C", 0.5, 10],
["15P", "30C", "5P", 0.4, 10],
["10HL", "10P", "25HL", 0.5, 40],
["30HL", "40P", "5HL", 0.5, 40],
["10HL", "40HL", "40P", 0.5, 40],
["20HL", "25HL", "30HL", 0.5, 30],
["25C", "30C", "40P", 0.5, 20],
["10P", "20C", "25P", 0.5, 10],
["20HL", "40C", "5P", 0.5, 30],
["15C", "25HL", "5C", 0.5, 30],
...
if every_term_random:
    add_txt = "_2020제외_4월26일_every_term_random"
else:
...
def main():
    ...
    for i in range(len(last_trains)):
        ...
        if every_term_random:
            ...
            et.selected_model_types = sorted(random.sample(model_pools, et.selected_num))
        ...
        profits.append((df['profit'].values - df['fee'].values).sum())
    ...
    if et.trading_9h:
        path = "9시시가에거래_ensemble3_" + str(et.pred_term) + et.target_type + "_reinfo_" + str(et.reinfo_th) + \
               "_" + str(et.model_reinfo_th) + "_losscut_" + str(et.profit.loss_cut) + "_" + ensemble_models + ".xlsx"
    else:
        path = "9시거래없음_ensemble3_" + str(et.pred_term) + et.target_type + "_reinfo_" + str(et.reinfo_th) + \
               "_" + str(et.model_reinfo_th) + "_losscut_" + str(et.profit.loss_cut) + "_" + ensemble_models + ".xlsx"
    ...
    return dates, closes, close_rates, profit_rates, np.array(profit_sum).std(), profit_product, ensemble_models, path
    ...
    if not every_term_random:
        ...
        for enm in models:
            ...
            for i in range(et.selected_num):
                ensemble_models = ensemble_models + enm[i] + "_"
            if not results == [] and [ensemble_models, str(et.reinfo_th), str(et.pred_term)] in np.array(results)[:,
                                                                                                :3].tolist():
                continue

            dates, closes, close_rates, profit_rates, std, profit_product, ensemble_models, path = main()

            results.append([ensemble_models, et.reinfo_th, et.pred_term, profit_rates[-1], std, profit_product])
            print([ensemble_models, et.reinfo_th, et.pred_term, profit_rates[-1], std, profit_product])
