...
# ensemble_test1과 다른 점은 reinfo_term만큼 pre_start_time을 back-off 한다.
...
start_time = '2022/10/16/09:00'
end_time = '2022/11/02/15:00'

global pre_start_time

last_train = '2022-10-15'

# 2022-2022-09-30 top 5 (2.2 ~ 2.6)
#selected_model_types = ['5C', '20C', '30HL']  # 2.38, 9.28, 1.918
#reinfo_th = 0.4
#model_reinfo_th = 0.4

# best reinfo, pred_term, 2022-01-01~10-31
pred_term = 5
#selected_model_types = ['5C', '10C', '10HL']
#selected_model_types = ['5C', '5HL', '10C']
selected_model_types = ['5C', '20C', '30HL']
#pred_term = 10
#selected_model_types = ['5C', '5HL', '10C']
#pred_term = 15
#selected_model_types = ['5C', '5HL', '10C']
...
def preprocessing():
    ...
    pre_start_time = df0['date'].values[df0.loc[df0['date'] >= start_time].index.min() - reinfo_term]

    _end_time = df0.loc[df0['date'] <= end_time].max()['date']
    _start_time = df0.loc[df0['date'] >= pre_start_time].min()['date']
    ...
    start_index = df0.loc[df0['date'] >= pre_start_time].index.min()
...
def predict():
    ...
    pred_start_time = df_pred['date'].values[df_pred.loc[df_pred['date'] >= start_time].index.min() - reinfo_term]

    start_index = df_pred.loc[df_pred['date'] >= pred_start_time].index.min()
    ...
    for i in range(selected_num):
        ...
        tm.mr.real_start_time = pred_start_time
    ...
    start_index = df.loc[df['date'] >= pred_start_time].index.min()
