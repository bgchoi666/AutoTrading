...
def run(start_date, current_price):
    ...
    for i in range(len(dates)):
        ...
        accu_rates.append(sum(profits[:i+1])/(purchase_price.mean()*1.25*250000*0.075) + 1)
    ...
    plt.xlabel('날짜')
    plt.ylabel('수익률')
    plt.title(start_date[:10] + '~현재, 수익률 변동')
    plt.plot(seq, accu_rates, label='3개의 딥러닝 모델들의 앙상블 전략')#, 'ro')
    plt.plot(seq, accu_buy_and_hold, label='선물지수등락 with lev. 13.3')#, 'bo')
