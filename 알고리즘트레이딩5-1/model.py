...
def create_model(conf):
    ...
    conf.checkpoint_path = conf.last_train+"/60M_input89_test"
...
def test(model, conf):
    ...
    # calculate accuracy
    c = 0
    s = list(test_prediction).count(1) + list(test_prediction).count(0)
...
def predict(model, conf):
    ...
    # last_train folder가 없으면 추가 (ensemble.py 실행시 발생)
