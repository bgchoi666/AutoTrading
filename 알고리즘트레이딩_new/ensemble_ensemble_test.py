...
eval_arr = pd.read_csv("eval_reflection_2022-01-01~2023-03-17_losscut0.01.csv", encoding='euc-kr').values[:, :5]
...
def predict():
    ...
    for i in range(selected_num):
        #s = eval_arr[selected_ensemble_types[i]][0].strip("[]").split(', ')
        #et.selected_model_types = s
        et.selected_model_types = [int(eval_arr[selected_ensemble_types[i]][0]), int(eval_arr[selected_ensemble_types[i]][1]), int(eval_arr[selected_ensemble_types[i]][2])]
        et.selected_checkpoint_path = [last_train + "/60M_" + str(et.selected_model_types[j]) + "_best" for j in
                                    range(selected_num)]
        et.reinfo_th = reinfo_th#eval_arr[selected_ensemble_types[i]][1]
        et.pred_term = pred_term#eval_arr[selected_ensemble_types[i]][2]
