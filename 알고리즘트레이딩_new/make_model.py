...
checkpoint_path_best = last_train+"/60M_" + str(pred_term) + "_best"
...
def preprocessing():
    ...
    train_start = df0.loc[df0['date'] >= start_time].index.min() - 1020
    if train_start < 19:
        train_start = 19
    train_end = df0.loc[df0['date'] <= end_time].index.max()

    df = df0[train_start:train_end+1].reset_index(drop=True)
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        ...
        if len(sys.argv) >= 6:
            ...
            checkpoint_path_best = last_train + "/60M_" + sys.argv[2] + "_best"
