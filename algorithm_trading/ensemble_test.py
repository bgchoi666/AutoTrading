...
pred_term = 5
reinfo_th = 0.39
model_reinfo_th = 0.39
ensembles = ['10HL', '40P', '5P']


selected_num = len(ensembles)
...
def predict():
    ...
    for i in range(selected_num):
        p = model_predict(ensembles[i], pred_input, pred_results, model_reinfo_th)
        r.append(p)
    r = np.array(r)
    ...
    dic = {'date': dates, 'result': pred, 'pred1': r[0], 'pred2': r[1], 'pred3': r[2], 'open': open, 'high': high, 'low': low, 'close': close}
