...
class MyWindow(QMainWindow, form_class):
    ...
    def timeout(self):
        ...
        if self.auto_gubun == 1:
             trading_state = "자동 test 거래 실행 중"
        elif self.auto_gubun == 0:
            trading_state = "수동 test 주문 대기 중"
        else:
    ...
    def predict(self, version):
        df0 = pd.read_csv("kospi200f_11_60M.csv", encoding='euc-kr')
        ...
        if version == 20:
            ...
            result_path = "../알고리즘트레이딩4/pred_83_results.csv"
        elif version == 41:
            # 알고리즘트레이딩 version4 ensemble predict
            ...
            result_path = "../알고리즘트레이딩_new/pred_83_results.csv"
        elif version == 'new1':
            # 알고리즘트레이딩 version new ensemble predict
        ...
        while True:
            ...
            if (datetime.datetime.now() - now).seconds > 60 * 5:
                ...
                print('예측 프로세스 실패... 시간 경과 5분... 프로그램 종료', chetime)
                exit(0)
            time.sleep(0.5)
    ...
    def show_state(self, version):
        ...
        if version == 20 or version == 21 or version == 22:
            trans_df = pd.read_csv("trans_2_test.csv", encoding="euc-kr")
        elif version == 40 or version == 41 or version == 42:
            trans_df = pd.read_csv("trans_4_test.csv", encoding="euc-kr")
        elif version == 'new0' or version == 'new1' or version == 'new2':
            trans_df = pd.read_csv("trans_new_test.csv", encoding="euc-kr")
        else:
        ...
        # 오늘 잔고 수량과 매수매도 상태
        now = datetime.datetime.now()
        trans_df_today = trans_df.loc[trans_df["거래일시"] >= now.strftime("%Y/%m/%d/00:00")]
    ...
    def calc_order_qty(self):
        ...
        if self.kiwoom.current_price != 0:
            for v in self.port:
                ...
                if v == 20 or v == 21 or v == 22:
                   trans_file_name = "trans_2_test.csv"
                elif v == 40 or v == 41 or v == 42:
                    trans_file_name = "trans_4_test.csv"
                elif v == 'new0' or v == 'new1' or v == 'new2':
                    trans_file_name = "trans_new_test.csv"
                else:
                    print("version error", version)
                    exit(0)
    ...
    def new_order(self, state, num):
        ...
        num2 = 0
        if num > self.kiwoom.total_quantity:
            num2 = num - self.kiwoom.total_quantity
            num = self.kiwoom.total_quantity
        ...
        if state == 1: # sell order
            print('매도주문 완료, 수량: ', num)
        elif state == 2: # buy order
            print('매수주문 완료, 수량: ', num)
        else:
            print("거래없음")
            return

        if not num2: # 잔여 거래 수량 없음
            if state == 1:
                gubun = "매도"
            elif state == 2:
                gubun = "매수"
            ...
            # 거래내역 trans file에 저장
            try:
                self.save_trans(gubun, purchase_num, purchase_price,  "trans_test.csv")
            except:
        ...
        purchase_price = self.kiwoom.current_price
        purchase_num = num # 원 거래 수량 

        # 시장가("3")로 주문
        if state == 1: # sell order
            print('매도주문 완료, 수량: ', num2) # 2차 거래 수량으로 거래
        elif state == 2: # buy order
            print('매수주문 완료, 수량: ', num2)
        else:
            print("거래없음")
            return
        ...
        # 거래내역 trans file에 저장
        try:
            self.save_trans(gubun, purchase_num, purchase_price)
        except:
            print("saving tran_test.csv error!!")
        print(gubun + ' 주문 완료... trans.csv 파일에 주문 내역 저장...')
    ...
    def check_balance(self):
        ...
        return round(extimated_deposit_assets / (float(self.kiwoom.current_price) * 250000 * 0.0795) / 3, 2)
    ...
    def save_trans(self, gubun, purchase_num, purchase_price):
        pred_date = datetime.datetime.now().strftime("%Y/%m/%d/%H:%M")

        # 주문 내역 거래내역 파일 (trans.csv) 에 저장
        trans_df = pd.read_csv('trans_test.csv', encoding='euc-kr')
        ...
        trans_df.to_csv("trans_test.csv", index=False, encoding="euc-kr")
