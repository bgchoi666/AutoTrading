...
end_times = [
    ...
    '2023/11/15/15:00', '2023/11/30/15:00', '2023/12/01/15:00', '2023/12/31/15:00',
    ...
    '2024/11/15/15:00', '2024/11/30/15:00', '2024/12/01/15:00', '2024/12/31/15:00',
]
...
folder = "앙상블실험_random/앙상블실험_" + term + "_" + "losscut" + str(et.profit.loss_cut) + "/"
...
def main():
    ...
    for i in range(et.selected_num):
        ensemble_models = ensemble_models + et.ensembles[i] + "_"
    ...
    for i in range(len(last_trains)):
        ...
        et.selected_num = len(et.ensembles)
        et.weights = [1 for i in range(et.selected_num)]
        et.selected_checkpoint_path = [et.last_train + "/60M_" + et.ensembles[i] + "_best" for i in range(et.selected_num)]
    ...
    profits_rates = profits.cumsum() / (closes.mean() * 250000 * 0.09) + 1#profits.cumsum() / 8000000 + 1
    MDD = calc_MDD(profits_rates)

    close_rates = (np.array(closes) - closes[0]) / closes[0] / 0.09 + 1#(np.array(closes) - closes[0]) * 250000 / 8000000 + 1
    ...
    return dates, closes, close_rates, profits_rates, MDD, profit_product, ensemble_models, path
...
if __name__ == "__main__":
    ...
    result_path = "eval_reflection_random/eval_reflection_" + term + "_" + "losscut" + str(et.profit.loss_cut) + ".csv"
    ...
    while len(results) < cnt:
        ...
        et.reinfo_th = random.randint(0, 50) / 100#random.sample(hit_ratios, 1)[0]
        et.model_reinfo_th = et.reinfo_th
        et.pred_term = random.randrange(5, 41, 5)#random.sample(eval_terms, 1)[0]

        for enm in ensembles:

            if random_ensemble:
                et.ensembles = sorted(random.sample(model_pools, 3))
            else:
                et.ensembles = enm
            ...
            dates, closes, close_rates, profit_rates, MDD, profit_product, ensemble_models, path = main()
            ...
            print("profit rates, std, profit_product, ensemble, reinfo, pred_term: ", profit_rates[-1], MDD, profit_product, et.ensembles, et.reinfo_th, et.pred_term)

            results.append([ensemble_models, et.reinfo_th, et.pred_term, profit_rates[-1], MDD, profit_product])
            pd.DataFrame(np.array(results), columns=['ensemble', 'self-reflection', 'pred_term', 'profit_rates', 'MDD', 'profit_product']).to_csv(result_path, index=False)
