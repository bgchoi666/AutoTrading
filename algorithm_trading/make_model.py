...
def create_model(mpdel_type):
    ...
    return model, pred_term, target_type, base1, base2, checkpoint_path_best
    ...
    if target_type == 'P':
        ...
        for i in range(len(df)):
            if i > len(df) - 1 - pred_term:
                ...
                if df.loc[i, '종가'] > df.loc[i+pred_term, '종가']*1.009:
                    target.append(1)
                elif df.loc[i, '종가'] < df.loc[i+pred_term, '종가']*0.991:
                    target.append(2)
                else:
                    target.append(0)
        df['target'] = target
        ...
        for i in range(len(df)):
            if i > len(df) - 1 - pred_term:
                ...
                if df.loc[i, base1] >= np.max(df.loc[i+1:i+pred_term-1, base1].values):
                    target.append(1)
                elif df.loc[i, base2] <= np.min(df.loc[i+1:i+pred_term-1, base2].values):
                    target.append(2)
                else:
                    target.append(0)
        df['target'] = target
...
def train(model, num):
    ...
    # 전체 dataframe에서 randoen으로 10%를 valid dataframe으로 사용하고 나머지는 train용
    valid_df = norm_df.iloc[:train_end_index].sample(n=int(len(norm_df)*0.1))
    ...
    # create valid data
    valid_data = valid_df.drop('date', axis=1, inplace=False).values
...
def test(model):
    ...
    # create test data
    test_data = norm_df.iloc[train_end_index:].values
    ...
    # calculate accuracy
    c = 0
    s = len([test_prediction[i] for i in range(len(test_prediction)) if test_prediction[i] == 1 or test_prediction[i] == 2])#len(test_prediction)
...
def predict(model):
    ...
    # self-reflection of the predictions
    mr.th = reinfo_th
    mr.target_type = target_type
    mr.pred_term = pred_term
    pred_results[:, 1] = mr.reinfo(pred, pred_results)
...
if __name__ == "__main__":
    ...
    model, pred_term, target_type, base1, base2, checkpoint_path_best = create_model(model_type)
