...
from PyQt5.QtWidgets import QApplication, QMainWindow, QDialog, QGridLayout, QLabel, QMessageBox, QTableWidget, \
...
class kiwoom(QAxWidget):
    ...
    def call_stock(self, sRQName, sTrCode, code, news_idx):
        print(
            "call_stock 호출됨, sRQNAME = " + sRQName + ". sTrCode = " + sTrCode + ", code = " + code + ", news_idx = " + news_idx)
        # self.now_code = code
    ...
    def receiveTrData(self, screenNo, requestName, trcode, recordName, inquiry,
        ...
        if requestName == "매수요청":
            ...
            if "관리종목" in is_gwansim:
                ...
                시가총액 = self.dynamicCall("CommGetData(QString, QString, QString, int, QString)", trcode, "", requestName,
                                        0, "시가총액")
                ...
                if n_price >= self.buy_temp[code]['week_jonga']:
                    ...
                    if upper_per >= self.upper_percent:
                        msg = ""
                        msg += "==============================================\n"
                        msg += codeName + "(" + code + ")는 일주일전 매수가 " + str(
                            self.buy_temp[code]['week_jonga']) + " 보다 " + str(
                            self.upper_percent) + "% 더 올라서 매수하지 않음.(현재가 : " + str(n_price) + ")"
                        msg += "\n"
                        myWindow.set_logbox(msg)
                        buy_okok2 = False
                    else:
                ...
                if buy_okok1 == True and buy_okok2 == True:
                    ...
                    buy_end = "NO"
                    print("order_money start, self.order_money = " + str(
                        self.order_money) + ", self.buy_able_money = " + str(self.buy_able_money))
                    ...
                    if returnCode != 0:
                        ...
                        # print("매수성공2!")
                        post_data = {"type": "stock_buy_ok", "stock_id": self.user_id, "buy_code": code,
                                     "buy_cnt": str(buy_count), "buy_account": buy_account_number, "news_idx": temp[2],
                                     "buy_price": n_price}
                        # print("post_data = " + str(post_data))
            ...
            try:
                ...
                for key in range(cnt):
                    ...
                    종목코드 = self.dynamicCall("GetCommData(QString, QString, int, QString)",
                                            [trcode, rr, key, "종목코드"]).strip()
                    종목명 = self.dynamicCall("GetCommData(QString, QString, int, QString)",
                                           [trcode, rr, key, "종목명"]).strip()
                    보유수량 = float(self.dynamicCall("GetCommData(QString, QString, int, QString)",
                                                  [trcode, rr, key, "보유수량"]).strip())
                    현재가 = float(self.dynamicCall("GetCommData(QString, QString, int, QString)",
                                                 [trcode, rr, key, "현재가"]).strip())
                    평가금액 = float(self.dynamicCall("GetCommData(QString, QString, int, QString)",
                                                  [trcode, rr, key, "평가금액"]).strip())
                    손익금액 = float(self.dynamicCall("GetCommData(QString, QString, int, QString)",
                                                  [trcode, rr, key, "손익금액"]).strip())
                    손익율 = float(self.dynamicCall("GetCommData(QString, QString, int, QString)",
                                                 [trcode, rr, key, "손익율"]).strip())
                    매입금액 = float(self.dynamicCall("GetCommData(QString, QString, int, QString)",
                                                  [trcode, rr, key, "매입금액"]).strip())
            ...
            일자 = self.dynamicCall("CommGetData(QString, QString, QString, int, QString)", trcode, "", requestName, 0,
                                  "일자")
            print("일자 = " + str(일자))
            종가 = self.dynamicCall("CommGetData(QString, QString, QString, int, QString)", trcode, "", requestName, 0,
                                  "종가")
            # print("종가 = " + 종가)
            종가 = abs(int(종가.strip()))
            print("종가 = " + str(종가))
            ...
            print(str(self.buy_temp))
            self.call_stock(self.buy_temp[code]['sRQName'], self.buy_temp[code]['sTrCode'], code,
                            self.buy_temp[code]['news_idx'])
            '''
            print("일자 = " + 일자)
            print("종가 = " + 종가)
    ...
    def receiveRealData(self, sGubun, nItemCnt, sFIdList):
        print("receiveRealData 호출됨, sGubun = " + str(sGubun) + ", nItemCnt = " + str(nItemCnt) + ", sFIdList = " + str(
            sFIdList))

    # 이게 ReceiveTrData 보다 먼저 호출되드라...
    def receiveMsg(self, scr_no, rq_name, tr_code, msg):
        print(
            "receiveMsg 호출됨, scr_no = " + scr_no + ", rq_name = " + rq_name + ", tr_code = " + tr_code + ", msg = " + msg)
